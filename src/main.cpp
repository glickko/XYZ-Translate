...
QString humanizeText(QString text, const QString& origText, bool isWriter, bool enableSlangMode) {
    ...
    //slash removal
    if (!orig.contains('/') && text.contains('/')) {
        text = text.split('/').first().trimmed();
    }
...
public:
    ModelManagerDialog(QWidget* parent = nullptr) : QDialog(parent) {
        ...
        QLabel* descLabel = new QLabel("Download NMT models for strictly offline translation.\nExample: 'en' to 'id'. Requires internet for downloading."); descLabel->setObjectName("Subtext"); contentLayout->addWidget(descLabel);
...
public:
    SettingsDialog(QSettings& s, QWidget* parent = nullptr) : QDialog(parent), settings(s) {
        setWindowFlags(Qt::FramelessWindowHint | Qt::Dialog); setAttribute(Qt::WA_TranslucentBackground); setStyleSheet(GLOBAL_STYLE); setFixedSize(500, 760);
        QVBoxLayout* mainLayout = new QVBoxLayout(this); mainLayout->setContentsMargins(0, 0, 0, 0); GlassPanel* glass = new GlassPanel(this); mainLayout->addWidget(glass);
        ...
        // [AUTO-PATCH PROMPT] Paksa prompt strict no slashes
...
public slots:
    ...
    void fetchClipboardAsync(std::function<void(QString)> callback) {
        ...
        ReleaseModifiers(); HardwareKeyAction(VK_CONTROL, false); HardwareKeyAction(0x43, false); HardwareKeyAction(0x43, true); HardwareKeyAction(VK_CONTROL, true);

        QTimer* timer = new QTimer(this); timer->setInterval(15);
        int* attempts = new int(0);
        connect(timer, &QTimer::timeout, [=]() {
            (*attempts)++; QString result = clipboard->text();
            if (!result.isEmpty() || *attempts > 25) {
    ...
    void onWriterHotkeyTriggered() {
        ...
        fetchClipboardAsync([this](QString copiedText) {
            ...
            else DiagnosticHUD::showMessage("[ERROR] No text selected!", "#FF4757");
        });
    }
...
private:
    ...
    void executeAPI(const QString& mode, const QString& textToTranslate, bool useDirectTyping) {
        ...
        if (currentEngine == "GTX") {
            DiagnosticHUD::showMessage("⚡ GTX (Auto-Detect) Translating...");
                ...
                connect(reply, &QNetworkReply::finished, [=, this]() {
                    ...
                    if (reply->error() == QNetworkReply::NoError) {
                        ...
                        if (errDoc.isObject() && errDoc.object().contains("error")) {
                            realErrorMsg = errDoc.object()["error"].toObject()["message"].toString();
                        }
...
LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION) {
        ...
        if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) {
            bool ctrl = (GetAsyncKeyState(VK_CONTROL) & 0x8000) != 0; bool alt = (GetAsyncKeyState(VK_MENU) & 0x8000) != 0; bool shift = (GetAsyncKeyState(VK_SHIFT) & 0x8000) != 0; bool win = (GetAsyncKeyState(VK_LWIN) & 0x8000) != 0 || (GetAsyncKeyState(VK_RWIN) & 0x8000) != 0;
            ...
            if (writerMatch) { if (g_mainWindow) QMetaObject::invokeMethod(g_mainWindow, "onWriterHotkeyTriggered", Qt::QueuedConnection); return 1; }
            if (readerMatch) { if (g_mainWindow) QMetaObject::invokeMethod(g_mainWindow, "onReaderHotkeyTriggered", Qt::QueuedConnection); return 1; }

            if (ctrl && alt && pKeyBoard->vkCode == VK_RETURN) { if (g_mainWindow) QMetaObject::invokeMethod(g_mainWindow, "onSuperEnterTriggered", Qt::QueuedConnection); return 1; }
...
//i love made single file hehehe
#include "main.moc"
